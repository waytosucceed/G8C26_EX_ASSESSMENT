...
const uniqueKey = "G8_EX_Assessment";
...
function startTimer() {
  let timeLeft = 45 * 60; // 40 minutes in seconds
  ...
  const timerId = setInterval(() => {
    ...
    if (timeLeft === 300) {
      ...
      new Audio('./assets/sounds/submit.mp3').play();
      showWarningModal(false);
    }

    // Show "Time Over" when time runs out
    if (timeLeft <= 0) {
...
function showWarningModal(isTimeOver) {
  const modalHtml = `
...
function loadQuestion(index) {
  ...
  randomQuestion.options.forEach(function(option, idx) {
    ...
    if (option.image) {
      ...
      optionImage.style.width = "60%";
      optionImage.style.maxHeight ="100px"
      optionImage.style.height = "25%";
